  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Development Requests</h1>
      </div>
      
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">All Development Requests</h2>
                ...
                {devEntries.map((entry) => (
                  <tr key={entry._id}>
                      ...
                      {entry.technologies && entry.technologies.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {entry.technologies.map((tech, idx) => (
                            <span key={idx} className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 rounded-full text-xs">
                              {tech}
                            </span>
                          ))}
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      ...
                      <button 
                        onClick={() => handleDeleteConfirmation(entry._id, 'dev')}
